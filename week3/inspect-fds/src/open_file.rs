...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        let link_dir = format!("/proc/{}/fd/{}", pid, fd);
        let fd_info_link = format!("/proc/{}/fdinfo/{}", pid, fd);

        let link = fs::read_link(link_dir).ok()?;
        let name = Self::path_to_name(&link.to_string_lossy());
        let fd_info = fs::read_to_string(fd_info_link).ok()?;
